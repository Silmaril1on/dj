...
const AddClub = () => {
  ...
  const normalizeAdditionalField = (value) => {
      ...
      initialData:
        isEditMode && clubData
          ? {
              ...
              social_links: normalizeAdditionalField(clubData.social_links),
              residents: normalizeAdditionalField(clubData.residents),
          ...
          : {
              ...
              social_links: normalizeAdditionalField(
                formConfigs.addClub.initialData.social_links,
              ),
              residents: normalizeAdditionalField(
                formConfigs.addClub.initialData.residents,
              ),
            },
    }),
    [isEditMode, clubData],
  ...
  const fetchClubData = async (clubId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        dispatch(
          setError({ message: "Failed to fetch club data", type: "error" }),
        );
      }
    } catch (error) {
      dispatch(
        setError({ message: "Error fetching club data", type: "error" }),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to ${isEditMode ? "update" : "submit"} club`,
        );
      }
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 items-start">
      {user?.submitted_club_id && !isEditMode && !user?.is_admin ? (

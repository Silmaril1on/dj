  ...
  const formConfig = {
    ...
    initialData:
      isEditMode && eventData
        ? Object.fromEntries(
            Object.entries(formConfigs.addEvent.initialData).map(
              ([key, defaultValue]) => [
                key,
  ...
  const fetchEventData = async (eventId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        dispatch(
          setError({ message: "Failed to fetch event data", type: "error" }),
        );
      }
    } catch (error) {
      dispatch(
        setError({ message: "Error fetching event data", type: "error" }),
      );
    } finally {
      setLoading(false);
    }
